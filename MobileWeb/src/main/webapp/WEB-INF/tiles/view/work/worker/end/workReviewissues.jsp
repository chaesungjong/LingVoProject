	...
	<div class="today_wrap">
		<c:forEach items="${result}" var="workIssueMsgListResponse" varStatus="status">		
			<div class="workStop_list">
				...
				<div class="select">
					<input type="radio" name="checkboxValues${status.index * 2 + 1}" id="inlineCheckbox${status.index * 2 + 1}" value="${workIssueMsgListResponse.issueDetail}${workIssueMsgListResponse.choice1}"><label for="inlineCheckbox${status.index * 2 + 1}"><c:if test="${empty workIssueMsgListResponse.choice1}"> 체크 </c:if>${workIssueMsgListResponse.choice1}</label>
